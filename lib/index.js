...
let HyDrawer = class HyDrawer extends applyMixins(RxLitElement, [ObservablesMixin, UpdateMixin, CalcMixin]) {
    constructor() {
        ...
        _HyDrawer_initialized.set(this, createResolvablePromise());
        ...
        this.upgrade = () => {
            ...
            const isScrimVisible$ = defer(() => {
                return __classPrivateFieldGet(this, _HyDrawer_translateX$, "f").pipe(map(translateX => translateX !== 0));
            });
            ...
            const translateX$ = __classPrivateFieldSet(this, _HyDrawer_translateX$, defer(() => {
                ...
                const moveTranslateX$ = move$.pipe(filterWhen(isSliding$), tap(() => (this.scrimClickable = false)), tap(({ event }) => event && this.noScroll && event.preventDefault()), withLatestFrom(start$, __classPrivateFieldGet(this, _HyDrawer_startTranslateX$, "f"), drawerWidth$), map(args => this.calcTranslateX(...args)));
                return merge(__classPrivateFieldGet(this, _HyDrawer_tweenTranslateX$, "f"), jumpTranslateX$, moveTranslateX$);
            }).pipe(share()), "f");
            __classPrivateFieldSet(this, _HyDrawer_startTranslateX$, translateX$.pipe(sample(start$)), "f");
            ...
            __classPrivateFieldSet(this, _HyDrawer_tweenTranslateX$, merge(willOpen$, animateTo$).pipe(withLatestFrom(translateX$, drawerWidth$), switchMap(([willOpen, translateX, drawerWidth]) => {
            ...
            __classPrivateFieldGet(this, _HyDrawer_initialized, "f").resolve(this);
        };
    ...
    get initialized() {
        return __classPrivateFieldGet(this, _HyDrawer_initialized, "f");
    }
    // HyDrawer
    getDrawerWidth() {
