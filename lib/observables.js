...
export class ObservablesMixin {
    getStartObservable() {
        return combineLatest([this.$.mouseEvents]).pipe(switchMap(([mouseEvents]) => {
            const touchstart$ = fromEvent(document, "touchstart", { passive: true }).pipe(filter(({ touches }) => touches.length === 1), map(({ touches }) => touches[0]));
    ...
    getMoveObservable(start$, end$) {
        return combineLatest([this.$.mouseEvents, this.$.preventDefault]).pipe(switchMap(([mouseEvents, preventDefault]) => {
            const touchmove$ = fromEvent(document, "touchmove", { passive: !preventDefault }).pipe(map(e => ((e.touches[0].event = e), e.touches[0])));
            const mousemove$ = !mouseEvents
                ? NEVER
                : fromEvent(document, "mousemove", { passive: !preventDefault }).pipe(subscribeWhen(merge(start$.pipe(mapTo(true)), end$.pipe(mapTo(false)))), map(e => ((e.event = e), e)));
    ...
    getEndObservable() {
        return combineLatest([this.$.mouseEvents]).pipe(switchMap(([mouseEvents]) => {
            ...
            const mouseup$ = !mouseEvents
                ? NEVER
                : fromEvent(document, "mouseup", { passive: true }).pipe(map(e => ((e.event = e), e)));
    ...
    getIsSlidingObservableInner(move$, start$) {
        if (this.threshold) {
            ...
            return move$.pipe(withLatestFrom(start$), map(([{ clientX, clientY, event }, { clientX: startX, clientY: startY }]) => {
                ...
                if (this.noScroll && isSliding && event)
